...
async function main() {
  ...
  if (!userIdentifier) {
    console.error(
      '❌ Usage: node scripts/debug-missing-steps.js <user_email_or_id>',
    );
    process.exit(1);
  }
  ...
  try {
    ...
    const user = await User.findOne({
      $or: [
        ...
        {
          _id: mongoose.Types.ObjectId.isValid(userIdentifier)
            ? userIdentifier
            : null,
        },
      ],
    ...
    console.log(
      `Account Created Date (YYYY-MM-DD): ${new Date(user.createdAt)
        .toISOString()
        .slice(0, 10)}`,
    );
    ...
    if (TARGET_DATE < accountCreatedDate) {
      console.log(
        `⚠️  WARNING: ${TARGET_DATE} is BEFORE account creation (${accountCreatedDate})`,
      );
    ...
    if (healthRecords.length === 0) {
      ...
      healthRecords.forEach(record => {
        console.log(`\n📅 ${record.date}`);
        console.log(
          `   Steps: ${record.steps || 0} (Bonus: ${record.bonusSteps || 0})`,
        );
    ...
    if (!gam) {
      ...
      console.log(
        `Last Passive Coin Time: ${gam.lastPassiveCoinTime || 'N/A'}`,
      );
    ...
    if (!aug7Record) {
      console.log(
        `❌ PROBLEM: No HealthActivity record exists for ${TARGET_DATE}`,
      );
      console.log('');
      ...
      if (TARGET_DATE < accountCreatedDate) {
        console.log(
          '   ⚠️  CONFIRMED: Backend would reject sync (date < account creation)',
        );
        console.log(
          `   Account created: ${accountCreatedDate}, Sync date: ${TARGET_DATE}`,
        );
      } else {
      ...
      console.log(
        '   • Check device logs: adb logcat | grep "EodSyncWorker\\|HealthSyncHelper"',
      );
      console.log('');
      console.log('3. Background sync stale data guards:');
      console.log(
        '   • Post-midnight guard (< 5 min) with implausible steps (889 > 820 at 4min)',
      );
      ...
      console.log(
        '   • Check server logs for sync attempts around midnight Aug 7→8',
      );
    } else {
      ...
      if (aug7Record.steps === 0) {
        ...
        console.log('Possible causes:');
        console.log(
          '• Sync request sent 0 steps due to client-side reading error',
        );
